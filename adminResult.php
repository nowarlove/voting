...
<body>
    ...
    <div class="content">
        ...
        while ($row_user = mysqli_fetch_array($run)) {
            $nPaslon = $row_user['Paslon'];
            $nNoUrutPaslon = $row_user['NoUrutPaslon'];
            $nVotes = $row_user['Votes'];
        ?>
            <!-- ===========php================================ -->
            <h4 style="text-transform: capitalize;"><b><?php echo $nNoUrutPaslon ?> :</b></h4>
            <div class="progress">
        ...
        <table class="table table-hover" style="overflow-x:auto;">
            ...
            $s = 0;
            while ($row_user = mysqli_fetch_array($run)) {
                $Paslon = $row_user['Paslon'];
                $NoUrutPaslon = $row_user['NoUrutPaslon'];
            ...
            ?>
                <tbody>
                    <tr style="text-transform: capitalize;">
                        <td><?php echo $s; ?></td>
                        <td><?php echo $Paslon; ?></td>
                        <td><?php echo $NoUrutPaslon; ?></td>
                        <td><?php echo $Votes; ?></td>
                        <td><img src="upload/<?php echo $Image; ?>" style="width: 100px;height: 100px; border-radius: 50%;"></td>
                        <td><?php echo $Status; ?></td>
                        <td><a href="adminResult.php?dcl=<?php echo $NoUrutPaslon ?>" class="btn btn-primary" title="Declared"><i class="fa fa-bullhorn"></i></a>
                            <a href="adminResult.php?udcl=<?php echo $NoUrutPaslon ?>" class="btn btn-danger" title="UnDeclared"><i class="fas fa-trash-alt"></i></a>
                        </td>
                    </tr>
                <?php }
            }else{
        ...
        if (isset($_GET['dcl'])) {
            ...
            $update = "update kandidat set Status='ON' where NoUrutPaslon='$dcl_PartyName'";
        ...
        if (isset($_GET['udcl'])) {
            ...
            $update = "update kandidat set Status='OFF' where NoUrutPaslon='$udcl_PartyName'";
            $nrun_dcl = mysqli_query($conn, $update);
            if ($nrun_dcl === true) {
