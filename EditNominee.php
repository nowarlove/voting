...
<body>
    <div class="registerform">
        ...
        if (isset($_GET['nedit'])) {
            ...
            $select = "select * from kandidat where Paslon='$edit_FullName'";
            $run = mysqli_query($conn, $select);

            $row_user = mysqli_fetch_array($run);
            $ePaslon = $row_user['Paslon'];
            $eNoUrutPaslon = $row_user['NoUrutPaslon'];
            $eImage = $row_user['Image'];
        }
        ?>
        <form action="" method="POST" enctype="multipart/form-data">
            <img src="upload/<?php echo $eImage; ?>" alt="Admin" class="rounded-circle" width="150"><br>
            <input type="text" name="Paslon" value="<?php echo $edit_FullName; ?>" required placeholder="Nominee">
            <input type="text" name="NoUrutPaslon" value="<?php echo $eNoUrutPaslon; ?>" required placeholder="Nominee Number"><br>
    ...
    if (isset($_POST['submit'])) {
        ...
        $update = "update kandidat set Paslon='$Paslon',NoUrutPaslon='$NoUrutPaslon',Image='$Image' where Paslon='$edit_FullName'";
